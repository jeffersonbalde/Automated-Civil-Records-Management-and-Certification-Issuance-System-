...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // NEW: Enhanced duplicate detection with real-time checking
            function checkForDuplicates() {
                ...
                if (childFirstName && childLastName && dob) {
                    ...
                    // Show checking state
                    duplicateMessage.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Checking for duplicate records...';
                    duplicateAlert.style.display = 'block';
                    duplicateAlert.className = 'alert alert-info';
                    similarRecordsList.style.display = 'none';
                    ...
                    const checkData = {
                        action: isEditMode ? 'check_duplicate_edit' : 'check_duplicate',
                    ...
                    fetch(endpoint, {
                        ...
                        .then((result) => {
                            if (result.is_duplicate) {
                                ...
                                duplicateMessage.innerHTML = `<strong>Duplicate Record Found!</strong><br>
                    A record for <strong>${childFirstName} ${childLastName}</strong> 
                    born on <strong>${new Date(dob).toLocaleDateString()}</strong> already exists.`;
                                duplicateAlert.className = 'alert alert-danger';
                                duplicateAlert.style.display = 'block';

                                // Show similar records if any
                                if (result.similar_records && result.similar_records.length > 0) {
                                    displaySimilarRecords(result.similar_records);
                                }
                                ...
                                // Similar records found (but not exact duplicates)
                                duplicateMessage.innerHTML = `<strong>Similar Records Found</strong><br>
                    Found ${result.similar_records.length} similar record(s). Please review to avoid duplicates.`;
                                duplicateAlert.className = 'alert alert-warning';
                                duplicateAlert.style.display = 'block';

                                displaySimilarRecords(result.similar_records);
                            } else {
                                // No duplicates or similar records found
                                duplicateAlert.style.display = 'none';
                            }
                        })
                        .catch((error) => {
                            console.error('Error checking duplicates:', error);
                            duplicateMessage.innerHTML = 'Error checking for duplicates. Please try again.';
                            duplicateAlert.className = 'alert alert-warning';
                        });
                } else {
                    // Hide duplicate alert if not enough data
